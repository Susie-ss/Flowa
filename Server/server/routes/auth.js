...
// 注册
router.post('/register', rateLimit('register', req => req.ip || req.connection.remoteAddress), (req, res) => {
...
router.post('/login', (req, res) => {
  ...
  db.run('UPDATE users SET last_login_ip = ?, last_login_at = ?, updated_at = ? WHERE id = ?', [
    clientIP,
...
router.get('/me', (req, res) => {
  ...
  try {
    ...
    const user = db.get('SELECT id, username, nickname, status, last_login_at, created_at FROM users WHERE id = ?', [payload.userId]);
...
router.post('/refresh', (req, res) => {
  ...
  try {
    ...
    // 获取用户信息
    const user = db.get('SELECT id, username FROM users WHERE id = ? AND status = 1', [payload.userId]);
    if (!user) {
...
router.put('/profile', (req, res) => {
  ...
  try {
    ...
    // 检查用户名是否已被占用
    const existing = db.get('SELECT id FROM users WHERE username = ? AND id != ?', [username, payload.userId]);
    if (existing) {
      return res.status(409).json({ error: '用户名已被占用' });
    }
    db.run('UPDATE users SET username = ?, updated_at = ? WHERE id = ?', [
...
router.get('/profile', (req, res) => {
  ...
  try {
    const payload = jwt.verify(token, config.JWT_SECRET);
    const user = db.get('SELECT id, username, nickname, status, created_at FROM users WHERE id = ?', [payload.userId]);
...
router.put('/nickname', (req, res) => {
  ...
  try {
    ...
    db.run('UPDATE users SET nickname = ?, updated_at = ? WHERE id = ?', [
