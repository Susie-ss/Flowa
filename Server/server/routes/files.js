...
router.get('/*', (req, res) => {
  ...
  // 3. 权限通过，读取文件
  const fullPath = path.join(__dirname, '..', '..', 'previewCache', 'projects', String(projectId), filePath);
  
  // 安全检查：防止路径遍历
  const resolvedPath = path.resolve(fullPath);
  const baseDir = path.resolve(path.join(__dirname, '..', '..', 'previewCache', 'projects', String(projectId)));
  ...
  if (!fs.existsSync(resolvedPath)) {
    ...
    if (ext === '.html') {
      ...
      const projData = db.get('SELECT pages_json FROM projects WHERE id = ?', [projectId]);
