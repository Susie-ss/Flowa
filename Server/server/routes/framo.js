...
// Framo: POST /api/sketch/import — 上传并解析 Sketch 文件
router.post('/sketch/import', async function(req, res) {
  try {
    ...
    if (!file.name.toLowerCase().endsWith('.sketch')) {
      return res.status(400).json({ ok: false, error: '仅支持 .sketch 文件' });
    }
    ...
    // 安全校验
    if (entries.length > 100000) throw new Error('Sketch 文件条目过多');
    var docEntry = entries.find(function(e) { return e.entryName === 'document.json'; });
    if (!docEntry) throw new Error('缺少 document.json');

    var document = JSON.parse(docEntry.getData().toString('utf8'));
    var pageEntries = entries.filter(function(e) { return /^pages\/[^/]+\.json$/i.test(e.entryName); });
    var pages = pageEntries.map(function(e) { return JSON.parse(e.getData().toString('utf8')); });

    var parsed = parseSketchDocument(document, pages);
    var crypto = require('crypto');
    var libraryId = 'lib-sketch-' + crypto.createHash('sha1').update(file.data).digest('hex').slice(0, 12);
    ...
    var library = {
      ...
      previewResult: { engine: 'json-fallback', exported: 0 },
    ...
    try {
      ...
      var DATA_FILE = path.join(__dirname, '..', 'data', 'sketch-libraries.json');
      fs.mkdirSync(path.dirname(DATA_FILE), { recursive: true });
      var existing = [];
      try { existing = JSON.parse(fs.readFileSync(DATA_FILE, 'utf8')); } catch(e) {}
      existing.unshift(library);
...
router.post('/ai/generate', function(req, res) {
  var prompt = req.body.prompt || '';
  var libraryId = req.body.libraryId || '';
  var library = LIBRARIES.find(function(l) { return l.id === libraryId; }) || LIBRARIES[0];
  var layout = buildLayout(prompt, library);
