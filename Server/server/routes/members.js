...
router.get('/:projectId/members', (req, res) => {
  try {
    // 检查权限（仅owner可查看成员列表）
    const project = db.get('SELECT owner_id FROM projects WHERE id = ?', [req.params.projectId]);
...
// 邀请协作（输入用户名）
router.post('/:projectId/invite', rateLimit('invite', req => req.ip || req.connection.remoteAddress), (req, res) => {
  ...
  try {
    ...
    // 查找要邀请的用户
    const targetUser = db.get('SELECT id FROM users WHERE username = ? AND status = 1', [username]);
    if (!targetUser) {
...
router.delete('/:projectId/members/:uid', (req, res) => {
  try {
    // 检查权限（仅owner可移除成员）
    const project = db.get('SELECT owner_id FROM projects WHERE id = ?', [req.params.projectId]);
    if (!project) return res.status(404).json({ error: '项目不存在' });
