...
router.post('/', (req, res) => {
  ...
  // 检查是否已存在同名产品线（仅限本用户）
  const existing = db.get('SELECT id FROM product_lines WHERE name = ? AND (owner_id = ? OR owner_id IS NULL)', [name, req.user.userId]);
  if (existing) {
  ...
  // 获取最大 sort_order
  const maxSort = db.get('SELECT MAX(sort_order) as max FROM product_lines');
...
router.delete('/:id', (req, res) => {
  ...
  try {
    // 移除项目关联（不删除项目本身）
    db.run('DELETE FROM project_product_lines WHERE product_line_id = ?', [id]);
    // 删除产品线
    db.run('DELETE FROM product_lines WHERE id = ?', [id]);
    res.json({ success: true });
...
// 从产品线移除项目
router.delete('/:lineId/remove-project/:projectId', (req, res) => {
  try {
    db.run(
