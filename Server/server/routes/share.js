...
// 通过分享链接访问项目（只读）
router.get('/:token', optionalAuthMiddleware, (req, res) => {
  const { token } = req.params;
  const { pwd } = req.query;
  
  // 安全地查询（兼容旧列）
  var { hasPassword, hasExpiry } = getShareCols();
...
// 验证分享密码（不返回项目数据）
router.post('/:token/verify-password', (req, res) => {
  const { token } = req.params;
...
// 重新生成 share_token
router.post('/regenerate/:projectId', authMiddleware, (req, res) => {
  const { projectId } = req.params;
  
  // 检查权限
  const project = db.get('SELECT owner_id FROM projects WHERE id = ?', [projectId]);
